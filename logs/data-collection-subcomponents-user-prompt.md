# Additional Sub-component Documentation

You need to document the following sub-components that are part of "Components/Data Collection":
- Callbacks
- Collection Actions
- Empty State
- Filters
...
IMPORTANT: Generate COMPREHENSIVE documentation for each sub-component. Even if the Storybook documentation is brief, use your knowledge to provide complete, practical information.
...
5. ALWAYS add a reference at the end saying: "For more details, see ./references/[component-name].md"
...
Now generate detailed documentation for each sub-component. For each one:
- Include a clear, detailed description (2-3 sentences minimum)
- List ALL props with their types, whether they're required, and what each does
- Provide at least one complete, runnable code example
- Mention any related components or hooks
- If the Storybook docs are sparse, infer reasonable details from component names and context
- ALWAYS add this line at the end: "For more details, see ./references/[component-name].md"

IMPORTANT: Generate the output as separate reference files. For each sub-component, output:
---
...
Sub-components to document (generate comprehensive content for each):
### Callbacks
...
IMPORTANT: Output should be formatted as multiple reference files, one for each sub-component, with YAML frontmatter containing the title.
